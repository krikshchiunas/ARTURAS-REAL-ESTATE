...
| URL | File | What |
|---|---|---|
| `/{lang}` | `app/[lang]/page.tsx` | WebGL-главная — куб над водой, 6 глав по скроллу |
| `/{lang}/about` | `.../about/page.tsx` | Hero, манифест, паспорт, одометры, карточки процесса |
...
Навигация: **Projects** ведёт в реестр `/projects`, оттуда — вход в 3D-карту.
`/map/{slug}` закрыт от индексации, canonical ведёт на карточку объекта: для
поиска это один и тот же объект, показанный двумя способами.
